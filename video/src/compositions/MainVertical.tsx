import React from 'react';
import { AbsoluteFill } from 'remotion';
import { MainScenes } from './MainScenes';
import { SIZE_16x9, SIZE_9x16, colors, fonts } from '../lib/tokens';
import { BrandMark } from '../components/BrandMark';
import { Subtitle } from '../components/Subtitle';
import { ProgressBar } from '../components/ProgressBar';
import { subtitles } from '../lib/script';

/**
 * 9:16 portrait variant — 1080×1920 — 30 fps.
 *
 * Strategy:
 *   - Top band: DOC-RENAME wordmark + short tagline.
 *   - Middle: the master visual timeline, scaled to fit width.
 *   - Bottom band: bigger subtitles + CTA strap.
 *
 * Visual timeline (MainScenes) is reused as-is.
 */
export function MainVertical() {
  const target = SIZE_9x16;
  const scale = target.width / SIZE_16x9.width; // 0.5625
  const scaledHeight = SIZE_16x9.height * scale; // 607.5
  const verticalBand = (target.height - scaledHeight) / 2; // 656.25

  // Reserve more for the bottom band (subtitles) and less for the top header.
  const topBand = Math.round(verticalBand * 0.55);
  const bottomBand = Math.round(verticalBand * 1.45);

  // Re-fit the scaled stage between the two bands.
  const stageTop = topBand;
  const stageHeight = target.height - topBand - bottomBand;
  const stageScale = stageHeight / SIZE_16x9.height;

  return (
    <AbsoluteFill style={{ background: colors.paper }}>
      {/* Top band — ink */}
      <div
        style={{
          position: 'absolute',
          top: 0,
          left: 0,
          right: 0,
          height: topBand,
          background: colors.ink,
          color: colors.paper,
          display: 'flex',
          flexDirection: 'column',
          justifyContent: 'center',
          alignItems: 'center',
          padding: 24,
          gap: 8,
        }}
      >
        <BrandMark size="md" inverted />
        <span
          style={{
            fontFamily: fonts.sans,
            fontSize: 22,
            color: 'rgba(247, 243, 234, 0.78)',
            fontWeight: 500,
            letterSpacing: '-0.005em',
          }}
        >
          Renommage documentaire pro
        </span>
      </div>

      {/* Scaled visual timeline */}
      <div
        style={{
          position: 'absolute',
          top: stageTop,
          left: (target.width - SIZE_16x9.width * stageScale) / 2,
          width: SIZE_16x9.width,
          height: SIZE_16x9.height,
          transform: `scale(${stageScale})`,
          transformOrigin: 'top left',
          overflow: 'hidden',
          background: colors.paper,
          borderTop: `1px solid ${colors.line}`,
          borderBottom: `1px solid ${colors.line}`,
        }}
      >
        <MainScenes />
      </div>

      {/* Bottom band — paper, with subtitle area + footer */}
      <div
        style={{
          position: 'absolute',
          bottom: 0,
          left: 0,
          right: 0,
          height: bottomBand,
          background: colors.paper,
          borderTop: `2px solid ${colors.gold}`,
        }}
      >
        {/* Footer URL strap */}
        <div
          style={{
            position: 'absolute',
            bottom: 32,
            left: 0,
            right: 0,
            textAlign: 'center',
            fontFamily: fonts.sans,
            fontSize: 26,
            color: colors.ink,
            fontWeight: 700,
            letterSpacing: '0.005em',
          }}
        >
          doc-rename.com
        </div>
      </div>

      {/* Subtitles (vertical = bigger + positioned in the bottom band) */}
      <Subtitle cues={subtitles} vertical />
      <ProgressBar />
    </AbsoluteFill>
  );
}
